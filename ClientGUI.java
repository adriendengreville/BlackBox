...
public class ClientGUI extends JFrame implements ActionListener {
	...
	//ATTRIBUTS
		private JFrame frame;									//la fenétre principale
		...
		final int port = 1664;									//numéro de port sur lequel on se connecte
		...
		Font tmpFont = null;									//les polices de caractéres custom qui seront inclues en ressources
		Font customLabels = null;
		Font customTitle = null;
	
		int pX, pY;												//les variables pour le déplacement de la fenétre (positions actuelles)
	...
	//MéTHODES
		...
		private void initialize() {						//initialisation de la GUI du client
			...
			/*
			 * Le reste de cette procédure est générée par windows builder et légérement mis en forme, sauf indications contraires
			 */
...
//			PromptSupport.setPrompt("IP", ipField);
		...
		public String getPassword(){	//récup du mdp pour le client
		...
		void connectionFailed() {	//pour que le client puisse signaler un probléme de connection
		...
		public void actionPerformed(ActionEvent e) {	//gestion des boutons		    
			...
			if(o == quitNetwork && connected) {	//si éa vient du bouton de déconnexion
				client.sendMessage(new ChatMessage(ChatMessage.LOGOUT, ""));
			...
			if(connected) {				//si éa vient de l'autre bouton ('envoyer) et qu'on est connecté 
				client.sendMessage(new ChatMessage(ChatMessage.MESSAGE, messageBox.getText()));				
				messageBox.setText("");
				return;
			}
			
			if(o == joinNetwork) {		//si éa vient du bouton de connexion
				String username = pseudoField.getText().trim();			//on enléve les espaces autour du pseudo
				...
				String server = ipField.getText().trim();	//on enléve les espaces éventuels autour de l'adresse
				...
				ipField.setEditable(false);							//on régles les boutons et champs de l'UI pour qu'ils se comportent logiquement
