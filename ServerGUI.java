...
public class ServerGUI extends JFrame {
	...
	//MéTHODES
		...
		void appendEvent(String str) {		//écrit dans le champ des événements
			event.append(str);
			event.setCaretPosition(event.getText().length() - 1);
		}//appendEvent
		
		public void bindStartStop() {	//transmet les états du bouton START/STOP
			if(server != null) { //si le serveur existe c'est qu'il tourne, donc on doit l'arréter
				server.stop();
				server = null;
				running = false;
		...
		public void windowClosing(WindowEvent e) {	//permet une fermeture propre du serveur lorsque la fenétre est fermée
			// if my Server exist
			if(server != null) {
				try {
		...
		class ServerRunning extends Thread { //thread serveur
			public void run() {
