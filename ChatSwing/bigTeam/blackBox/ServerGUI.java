...
//METHODES-------------------------------------------------------------------------------------------------
	...
	private void initUI(){
		...
		//DRAG IT START
			...
			frame.addMouseMotionListener(new MouseMotionAdapter() {					//pour déplacer les fenètre
	...
	public void appendRoom(String str) {		//écrit dans le champ de chat
		chat.append(str);
		chat.setCaretPosition(chat.getText().length() - 1);
	}//appendRoom
	
	public void appendEvent(String str) {		//écrit dans le champ des évènements
		event.append(str);
	...
	public void bindStartStop() {	//transmet les états du bouton START/STOP
		...
		running = true;
		new ServerRunning().start();	//et on démarre le thread du serveur
		startStop.setText("Stop");
		password.setEditable(false);
	}//actionPerformed

	public void windowClosing(WindowEvent e) {	//permet une fermeture propre du serveur lorsque la fenètre est fermée
		// if my Server exist
		if(server != null) {
			try {
				server.stop();			// demande au serveur de terminer la connection
			}
	...
	private class ServerRunning extends Thread { //thread serveur
		public void run() {
			appendEvent("Le serveur a été démarré.\n");
			
			server.start();         // s'exécute tant que le serveur fonctionne

			startStop.setText("Start");
			password.setEditable(true);
			appendEvent("Le serveur a été arrété.\n");
			server = null;
		}
	}//ServerRunning
